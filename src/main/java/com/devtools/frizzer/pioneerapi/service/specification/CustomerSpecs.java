package com.devtools.frizzer.pioneerapi.service.specification;
...
public class CustomerSpecs {
    ...
    public static Specification<Customer> nameLike(String name) {
        return (root, query, builder) -> checkNull(builder.like(root.get("username"), name + "%"), builder, name);
    }

    public static Specification<Customer> phoneEquals(String phone) {
        return (root, query, builder) -> checkNull(builder.equal(root.get("phoneData").get("phoneNumber"), phone), builder, phone);
    }

    public static Specification<Customer> emailEquals(String email) {
        return (root, query, builder) -> checkNull(builder.equal(root.get("emailData").get("email"), email), builder, email);
    }
