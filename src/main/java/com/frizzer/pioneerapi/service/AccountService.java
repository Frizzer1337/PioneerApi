...
public class AccountService {
    ...
    public BigDecimal transfer(long senderId, long recipientId, BigDecimal amount) {
        log.info("Начат перевод денег от пользователя {} к пользователю {} сумма перевода {}", senderId, recipientId, amount);
        ...
        Account sender = accountRepository.findByCustomerIdForUpdate(senderId)
                                        .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Отправитель не найден"));
        ...
        log.info("Закончен перевод денег от пользователя {} к пользователю {} сумма перевода {}", senderId, recipientId, amount);
